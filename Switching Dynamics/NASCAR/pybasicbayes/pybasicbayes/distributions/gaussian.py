...
from NASCAR.pybasicbayes.pybasicbayes.abstractions import GibbsSampling, MeanField, MeanFieldSVI, Collapsed, MaxLikelihood, MAP, Tempering
from NASCAR.pybasicbayes.pybasicbayes.distributions.meta import _FixedParamsMixin
from NASCAR.pybasicbayes.pybasicbayes.util.stats import sample_niw, invwishart_entropy, sample_invwishart, invwishart_log_partitionfunction, getdatasize, flattendata, getdatadimension, combinedata, multivariate_t_loglik, gi, niw_expectedstats
...
class _GaussianBase(object):
    ...
    def log_likelihood(self,x):
        try:
            ...
            out = -1./2. * (xs.T,xs.T).sum(axis=1) - D/2*np.log(2*np.pi) \
    ...
    def expected_log_likelihood(self, x=None, stats=None):
        ...
        if x is not None:
            ...
            return self._loglmbdatilde()/2 - D/(2*kappa_n) - nu_n/2 * \
                (xs.T,xs.T).sum(axis=1) - D/2*np.log(2*np.pi)
        else:
