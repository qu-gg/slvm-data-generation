...
if __name__ == '__main__':
    ...
    for dataset, class_id in zip(["double_pendulum_2g", "double_pendulum_3g", "double_pendulum_4g"], [0, 1, 2]):
        num_steps = 30
        dt = 0.1
        steps_per_dt = 1
        ...
        """ Data Generation """
        dataset_path = f'double_pendulum3/{dataset}/'
        ...
        if not os.path.exists(f"double_pendulum3/double_pendulum_{class_id}/"):
            os.mkdir(f"double_pendulum3/double_pendulum_{class_id}/")
            
        np.savez(f"double_pendulum3/double_pendulum_{class_id}/train.npz", image=images[:num_train], state=states[:num_train], label=labels[:num_train])
        np.savez(f"double_pendulum3/double_pendulum_{class_id}/val.npz", image=images[num_train:num_train + num_val], state=states[num_train:num_train + num_val], label=labels[num_train:num_train + num_val])
        np.savez(f"double_pendulum3/double_pendulum_{class_id}/test.npz", image=images[num_train + num_val:], state=states[num_train + num_val:], label=labels[num_train + num_val:])
        shutil.rmtree(f"double_pendulum3/{dataset}/")
