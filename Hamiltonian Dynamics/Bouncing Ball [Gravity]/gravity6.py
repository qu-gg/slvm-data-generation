...
if __name__ == '__main__':
    ...
    for idx, g in enumerate(gs):
        ...
        if not os.path.exists(f"{base_path}/grav_{idx}/"):
            os.mkdir(f"{base_path}/grav_{idx}/")

        # Save sets
        np.savez(os.path.abspath(f"{base_path}/grav_{idx}/train.npz"), image=train_images, state=train_states, label=train_classes)
        np.savez(os.path.abspath(f"{base_path}/grav_{idx}/val.npz"), image=val_images, state=val_states, label=val_classes)
        np.savez(os.path.abspath(f"{base_path}/grav_{idx}/test.npz"), image=test_images, state=test_states, label=test_classes)
