...
from simulation_lib.hamiltonian_systems import ideal_double_pendulum
from simulation_lib.hamiltonian_systems import ideal_mass_spring
from simulation_lib.hamiltonian_systems import ideal_pendulum
from simulation_lib.hamiltonian_systems import n_body
...
try:
  from simulation_lib.multiagent_dynamics import game_dynamics  # pylint: disable=g-import-not-at-top
...
def generate_full_dataset(
  ...
  folder = os.path.join(folder, dataset.lower())
  set_up_folder(folder, overwrite)

  cls, config = globals().get(dataset.upper())
  system = cls(**config())
  generate = functools.partial(generate_sample, system=system, dt=dt, num_steps=num_steps, steps_per_dt=steps_per_dt)
  pipeline = create_pipeline(generate, folder, num_train, num_test, return_generated_examples)
  return pipeline()
...
PENDULUM_2G = (
    ...
    lambda: dict(  #  pylint:disable=g-long-lambda
        m_range=utils.BoxRegion(0.5, 1.0),
...
PENDULUM_4G = (
    ...
    lambda: dict(  #  pylint:disable=g-long-lambda
        m_range=utils.BoxRegion(0.5, 1.0),
        g_range=utils.BoxRegion(4.0, 4.0),
        l_range=utils.BoxRegion(0.5, 1.0),
        radius_range=utils.BoxRegion(1.3, 2.3),
        num_colors=1,
    )
...
MASS_SPRING_1K = (
    ...
    lambda: dict(  #  pylint:disable=g-long-lambda
        k_range=utils.BoxRegion(1.0, 1.0),
...
MASS_SPRING_3K = (
    ideal_mass_spring.IdealMassSpring,
    lambda: dict(  #  pylint:disable=g-long-lambda
        k_range=utils.BoxRegion(3.0, 3.0),
        m_range=utils.BoxRegion(0.4, 1.0),
        radius_range=utils.BoxRegion(0.1, 1.0),
        uniform_annulus=False,
        randomize_canvas_location=True,
