...
class BallBox:
    ...
    def create_ball(self, radius=3):
        ...
        position = np.clip(position, self.dd + radius +1, self.res[0]-self.dd-radius-1).tolist()
...
if __name__ == '__main__':
    ...
    if not os.path.exists('bouncingball/'):
        os.makedirs('bouncingball/')

    print("Generating training sequences")
    cannon = BallBox(dt=0.25, res=(32*scale, 32*scale), init_pos=(16*scale, 16*scale), init_std=8, wall=None)
    ...
    # Condition 1: Check if any timestep contains all zeros
    # For a timestep to be all zeros, every element in that timestep must be zero
    all_zeros_condition = np.any(np.all(i == 0, axis=(2, 3)), axis=1)

    # Condition 2: Check if any timestep contains a row with all elements as 1
    # For a row to have all 1s, every element in that row must be 1
    row_all_ones = np.any(np.all(i == 1, axis=3), axis=2)  # Rows with all 1s per timestep
    row_all_ones_condition = np.any(row_all_ones, axis=1)

    # Combine both conditions
    bad_indices = all_zeros_condition | row_all_ones_condition
    ...
    # Save sets
    np.savez(os.path.abspath(f"bouncingball/train.npz"), image=train_images, state=train_states, label=train_classes)
    np.savez(os.path.abspath(f"bouncingball/val.npz"), image=val_images, state=val_states, label=val_classes)
    np.savez(os.path.abspath(f"bouncingball/test.npz"), image=test_images, state=test_states, label=test_classes)
