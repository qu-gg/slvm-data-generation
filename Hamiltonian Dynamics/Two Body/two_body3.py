...
if __name__ == '__main__':
    ...
    for dataset, class_id in zip(["two_body_1g", "two_body_2g", "two_body_3g"], [0, 1, 2]):
        ...
        """ Data Generation """
        dataset_path = f'two_body3/{dataset}/'
        ...
        if not os.path.exists(f"two_body3/two_body_{class_id}/"):
            os.mkdir(f"two_body3/two_body_{class_id}/")
            
        np.savez(f"two_body3/two_body_{class_id}/train.npz", image=images[:num_train], state=states[:num_train], label=labels[:num_train])
        np.savez(f"two_body3/two_body_{class_id}/val.npz", image=images[num_train:num_train + num_val], state=states[num_train:num_train + num_val], label=labels[num_train:num_train + num_val])
        np.savez(f"two_body3/two_body_{class_id}/test.npz", image=images[num_train + num_val:], state=states[num_train + num_val:], label=labels[num_train + num_val:])
        shutil.rmtree(f"two_body3/{dataset}/")
